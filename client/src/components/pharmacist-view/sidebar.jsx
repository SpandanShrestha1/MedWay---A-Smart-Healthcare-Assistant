...
function MenuItems({ setOpen }) {
  ...
  return (
    <nav className="mt-8 flex-col flex gap-2">
      {pharmacistSidebarMenuItems.map((menuItem) => (
        <div
          key={menuItem.id}
          onClick={() => {
          ...
          className="flex cursor-pointer text-xl items-center gap-2 rounded-md px-3 py-2 text-gray-700 hover:bg-gray-100 hover:text-black transition-colors"
        >
          {menuItem.icons}
...
function PharmacistSideBar({ open, setOpen }) {
  const navigate = useNavigate();
  return (
    <Fragment>
